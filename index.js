...
const init = async () => {
  ...
  try {
    ...
    _.each(loved, (lovedTrack) => {
      ...
      queue.push((async () => {
        try {
          const match = _.find(scanResult, (track) => track.Comment === lovedTrack.mbid || track.Comment === lovedTrack.url || (track.title === lovedTrack.title && track.artist === lovedTrack.artist));
          if (match) {
          ...
          if (ytScrape.youtubeUrl) {
            try {
              ...
              track = _.defaultsDeep(track, {
                ...
                comment: lovedTrack.mbid ? lovedTrack.mbid : lovedTrack.url, // Tag the mbid and LastFM URL so we can check it later for syncing purposes.
              });
            ...
            Logger.warn(`${asyncId} has no linked YouTube video -- skipping ...`);
          }
        } catch (errYtScrape) {
